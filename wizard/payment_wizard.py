...
class CreatePayment(models.TransientModel):
    ...
    installment_id = fields.Many2one(
        "installment.installment", string="Installment", required=True, readonly=True
    )
    ...
    def action_create_payment(self):
        ...
        if self.installment_id.amount - total_paid == 0:
            raise UserError(
                "The Installment is fully payed you can't add new payments."
            )
        else:
            if self.pay_amount > self.installment_id.amount - total_paid:
                raise ValidationError(
                    "The payment amount is greater than reminded on this installment which is {}".format(
                        self.installment_id.amount - total_paid
                    )
                )
            else:
                vals = {
                ...
                if (
                    ...
                    - sum(self.installment_id.payment_lines_ids.mapped("amount"))
                    == 0
                ):
