...
class Installment(models.Model):
    ...
    analytic_tags = fields.Many2many(
        comodel_name="account.account.tag", string="Account Tags"
    )
    ...
    currency_id = fields.Many2one(
        "res.currency", related="customer.property_product_pricelist.currency_id"
    )
    invoice_id = fields.Many2one(
        "account.move", help="The Invoice Linked to the installment"
    )
        ...
        for rec in self:
            if rec.state != "draft":
                raise ValidationError(
                    "You can only delete installments with Draft state."
                )
            else:
                if rec.payment_lines_ids:
                    raise UserError(
                        _(
                            "There are payments referring to this record you should delete them first."
                        )
                    )
                else:
    ...
    def action_paid(self):
        for rec in self:
            if rec.amount - sum(rec.payment_lines_ids.mapped("amount")) != 0:
                raise UserError(
                    "You must settle the installment first and it will automatically be in paid status."
                )
            else:
                rec.state = "paid"
        ...
        if vals.get("name", _("Draft")) == _("Draft"):  # get next sequence
            ...
            if vals.get("invoice_id"):
                total_invoice_amount = (
                    ...
                    .search([("id", "=", vals["invoice_id"])])
                    .amount_total
                )
                # print(total_invoice_amount)
                past_installment_total = sum(
                    self.env["installment.installment"]
                    .search([("invoice_id", "=", vals["invoice_id"])])
                    .mapped("amount")
                )
...
class InvoiceInstallments(models.Model):
    ...
    def action_create_installment(self):
        for rec in self:
            ...
            if total_amount_payment >= rec.amount_total:
                raise UserError(
                    _("All the invoice installments has been already created.")
                )
                return
            else:
                try:
                    form_view_id = self.env.ref(
                        "custom_installment.view_installment_form"
                    ).id
    ...
    def _get_count_installments(self):
        for rec in self:
            rec.count_installments = self.env["installment.installment"].search_count(
                [("invoice_id", "=", rec.id)]
            )
